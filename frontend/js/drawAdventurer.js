...
function fillAdventurersTable(adventurers){
  if(adventurers != null && adventurers.length){
    ...
    var adventurersTable = createElement("TABLE");
    ...
    body.appendChild(adventurersTable);
...
function createDebugButtons(){
  body.appendChild(createButton("startLevelUp", "Start Level-up Process"));
  body.appendChild(createButton("stopLevelUp", "Stop Level-up Process"));
  body.appendChild(createButton("deleteAdventurers", "Delete Adventurers"));

  //This is a debug button.
  getElementById("startLevelUp").addEventListener("click", function(){
    ...
    this.disabled = true;
    getElementById("stopLevelUp").disabled = false;
  });
  //This is a debug button.
  getElementById("stopLevelUp").addEventListener("click", function(){
    for(var timer in timers){
      clearInterval(timers[timer]);
    }
    timers = [];
    this.disabled = true;
    getElementById("startLevelUp").disabled = false;
    saveData();
  });
  //This is a debug button.
  //This process will be much more streamlined and will occur without a reload.
  getElementById('deleteAdventurers').addEventListener("click", function(){
...
function updateAdventurerTile(adventurer){
  var adventurerTable = getElementById(adventurer.name);
